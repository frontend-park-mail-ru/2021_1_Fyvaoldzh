...
import OneProfileView from "../OneProfileView/OneProfileView";
...
export default class ChangePageView {
  ...
  public oneProfileView: OneProfileView;
  ...
  constructor(
    ...
    this.oneProfileView = oneProfileView;
  ...
  async render(state: HistoryState) {
    ...
    if (state.page.includes("profile") && state.page !== routes.profile) {
      this.actions.oneProfileUpdateByHistory();
      return;
    }
  ...
  renderSignUp() {
    this.wrapper.style.background =
      'url("components/img/form-background.jpg") no-repeat top / cover';
  ...
  changePage() {
    ...
    if (
      ...
      window.scroll(0, 0);
      this.actions.updateOneProfile(Number(currentUrl.pathname.substr(8)));
      return;
    }
