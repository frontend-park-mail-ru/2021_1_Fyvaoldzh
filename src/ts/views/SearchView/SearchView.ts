...
export default class SearchView extends ProfilesBaseView {
  ...
  renderSearchPage() {
    ...
    (document.getElementById('searchInput') as HTMLInputElement).value = searchData;

    document.getElementById('searchInput').addEventListener('change', this.handleSearch.bind(this));
  ...
  handleSearch() {
    const inputVal = (document.getElementById('searchInput') as HTMLInputElement).value;
    const { currentUrl } = this.globalStore.routerStore;

    window.history.pushState({ page: currentUrl.pathname, parameter: inputVal },
