...
const myProfileTemplate = require("Templates/profile/profile.pug");
const myProfileAboutTabTemplate = require("Templates/profile-about-tab/profile-about-tab.pug");
const myProfileSettingsTabTemplate = require("Templates/profile-settings-tab/profile-settings-tab.pug");
const myProfileEventsTabTemplate = require("Templates/profile-events-tab/profile-events-tab.pug");
...
export default class UserView {
      ...
      switch (error) {
        case "login":
          document
            ...
            .forEach((el) => (el.style.boxShadow = "0px 0px 10px 0px #CE0E50"));
          document.getElementById("loginError").innerText =
            INPUTS.login.errorMsg;
          break;
        ...
        case "wrongLoginOrPass":
          ...
          document
            ...
            .forEach((el) => (el.style.boxShadow = "0px 0px 10px 0px #CE0E50"));
          document.getElementById("passwordError").innerText =
            SERVER_ERRORS.WRONG_LOGIN_OR_PASS;
          break;
  ...
  subscribeViews() {
    ...
    this.globalStore.eventBus.subscribe(
      ChannelNames.userUpdated,
      this.renderMyProfilePage.bind(this)
    );
    this.globalStore.eventBus.subscribe(
  ...
  buttonToggleHandler(e: MouseEvent) {
    ...
    if (
      ...
      target.classList.contains("tab-inactive")
    ) {
    ...
    if (
      target instanceof HTMLButtonElement &&
      target.classList.contains("button-inactive")
    ) {
